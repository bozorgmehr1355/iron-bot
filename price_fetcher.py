        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(urls[product_key], headers=headers, timeout=15) as resp:
                if resp.status == 200:
                    ...
                    for sel in ['.product-price', '.price', '.current-price', '.price-value']:
...
async def get_iran_prices():
    ...
    prices = {
        "concentrate": {"name": "کنسانتره سنگ آهن", "ice": f"{ime['concentrate']:,}" if ime['concentrate'] else "۴,۲۰۰,۰۰۰ - ۴,۸۰۰,۰۰۰", "free_market": "۵,۴۰۰,۰۰۰ - ۵,۸۰۰,۰۰۰", "factory": "گل گهر: ۴,۳۰۰,۰۰۰ | مرکزی: ۴,۶۰۰,۰۰۰", "unit": "تن"},
        "pellet": {"name": "گندله", "ice": f"{ime['pellet']:,}" if ime['pellet'] else "۶,۲۰۰,۰۰۰ - ۶,۸۰۰,۰۰۰", "free_market": "۶,۸۰۰,۰۰۰ - ۷,۵۰۰,۰۰۰", "factory": "گل گهر: ۶,۴۰۰,۰۰۰ | چادرملو: ۶,۳۰۰,۰۰۰", "unit": "تن"},
        ...
        "billet": {"name": "شمش فولادی", "ice": f"{ime['billet']:,}" if ime['billet'] else f"{int(billet*0.95):,} - {int(billet*1.02):,}", "free_market": f"{int(billet*0.98):,} - {int(billet*1.05):,}", "factory": f"اصفهان: {billet:,} | یزد: {billet-500:,} | قزوین: {billet-1500:,}", "unit": "کیلو"},
        "rebar": {"name": "میلگرد", "ice": f"{ime['rebar']:,}" if ime['rebar'] else f"{int(rebar*0.95):,} - {int(rebar*1.02):,}", "free_market": f"{int(rebar*0.98):,} - {int(rebar*1.05):,}", "factory": f"ذوب آهن: {rebar:,} | امیرکبیر: {rebar+1000:,}", "unit": "کیلو"},
